...
from bodhion.retrieval.vector.main import (
    VectorDBBase,
    VectorItem,
    SearchResult,
    GetResult,
)
from bodhion.retrieval.vector.utils import process_metadata
...
class ChromaClient(VectorDBBase):
    ...
    def reset(self):
        ...
        # client.reset() clears all internal state; no need to delete the
        # filesystem and reinitialize — doing so causes the Rust backend to
        # enter an unrecoverable state (missing 'bindings' attribute) on Windows.
        self.client.reset()
