...
class VectorSearchRetriever(BaseRetriever):
    ...
    def _get_relevant_documents(
        ...
        raise NotImplementedError(
            "VectorSearchRetriever is async-only. Use aget_relevant_documents() "
    ...
    async def _aget_relevant_documents(
        ...
        *,
        ...
        for idx in range(len(ids)):
            ...
            # Flatten embedding_config so callers can read embedding_model directly
            # without having to know the nested key structure.
            if "embedding_model" not in metadata:
                embedding_cfg = metadata.get("embedding_config") or {}
                if isinstance(embedding_cfg, str):
                    try:
                        import ast
                        embedding_cfg = ast.literal_eval(embedding_cfg)
                    except Exception:
                        embedding_cfg = {}
                model = embedding_cfg.get("model") or embedding_cfg.get("engine") or "unknown"
...
def query_doc(
    ...
    try:
        ...
        if result:
            ...
            if os.environ.get("GLOBAL_LOG_LEVEL", "").upper() == "DEBUG":
                log.info("query_doc:result:detail " + __import__("json").dumps({
                    "ids": result.ids,
...
async def query_collection(
    ...
    log.debug(
        f"[RAG CP2c] query_collection embeddings generated | "
        f"embedding_count={len(query_embeddings)} collections={list(collection_names)}"
    )
    log.debug(
    ...
    try:
        ...
        for _ in range(5):
            ...
            if "data" in data:
                ...
                raise Exception("Something went wrong :/")
        raise Exception("Azure OpenAI embedding failed after 5 retries (rate limited).")
    except Exception as e:
        ...
        async def async_embedding_function(query, prefix=None, user=None):
            if isinstance(query, list):
                ...
                if enable_async:
                    ...
                    # On Windows, select() has a hard 512-FD limit — cap unlimited (0)
                    # to 3 concurrent requests. 20 simultaneous connections flood Ollama's
                    # accept() backlog and cause WSAETIMEDOUT on all of them at once.
                    # On Linux/macOS, epoll/kqueue has no FD limit so 0 stays unlimited.
                    _limit = concurrent_requests if concurrent_requests > 0 else (
                        3 if sys.platform == "win32" else 0
                    )

                    if _limit:
                        semaphore = asyncio.Semaphore(_limit)
                ...
                for batch_idx, batch_embeddings in enumerate(batch_results):
                    if not isinstance(batch_embeddings, list):
                        raise ValueError(
                            f"Embedding batch {batch_idx} returned an invalid result "
                            f"({type(batch_embeddings).__name__}). "
                            "The embedding service may be unavailable or misconfigured."
                        )
...
def get_reranking_function(reranking_engine, reranking_model, reranking_function):
    if reranking_function is None:
        return None
    # CrossEncoder token limit is 512; truncate to ~1600 chars (~400 tokens at 4 chars/token)
    # to prevent silent score degradation on long chunks.
    _MAX_CHARS = 1600

    if reranking_engine == "external":
        return lambda query, documents, user=None: reranking_function.predict(
            [(query, doc.page_content[:_MAX_CHARS]) for doc in documents], user=user
        )
    else:
        return lambda query, documents, user=None: reranking_function.predict(
            [(query, doc.page_content[:_MAX_CHARS]) for doc in documents]
        )
...
async def get_sources_from_items(
    ...
    log.debug(
        f"[RAG CP2b] get_sources_from_items entry | items_count={len(items)} "
        f"queries={queries} full_context={full_context} k={k} r={r} "
        f"hybrid_search={hybrid_search} reranking_function_present={reranking_function is not None}"
    )
    ...
    for item in items:
        ...
        log.debug(
            f"[RAG CP2b] processing item | type={item.get('type')} id={item.get('id')} "
            f"name={item.get('name')} collection_name={item.get('collection_name')} "
            f"collection_names={item.get('collection_names')} context={item.get('context')} "
            f"legacy={item.get('legacy')}"
        )
        ...
        log.debug(
            f"[RAG CP2b] item result | query_result_present={query_result is not None} "
            f"collection_names_resolved={list(collection_names)}"
        )

        if query_result:
            docs_inner = query_result.get("documents", [[]])[0] if query_result.get("documents") else []
            log.debug(
                f"[RAG CP2b] query_result docs | doc_count={len(docs_inner)} "
                f"has_metadatas={bool(query_result.get('metadatas'))} "
                f"has_distances={bool(query_result.get('distances'))}"
            )
            if "data" in item:
                del item["data"]
            query_results.append({**query_result, "file": item})

    log.debug(f"[RAG CP2b] query_results total={len(query_results)}")
    ...
    log.debug(
        f"[RAG CP2b] get_sources_from_items returning | sources_count={len(sources)} "
...
class RerankCompressor(BaseDocumentCompressor):
    ...
    def compress_documents(
        ...
        raise NotImplementedError(
            "RerankCompressor is async-only. Use acompress_documents() "
            "or call from an async context via ContextualCompressionRetriever.ainvoke()."
        )
