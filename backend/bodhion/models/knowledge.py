...
from bodhion.models.users import User, UserModel, Users, UserResponse
    ...
    def delete_all_knowledge(self, db: Optional[Session] = None) -> bool:
        with get_db_context(db) as db:
            try:
                ...
                db.query(Knowledge).delete(synchronize_session=False)
                db.commit()

                return True
            except Exception as e:
                log.exception(f"Failed to delete all knowledge: {e}")
                db.rollback()
                return False
