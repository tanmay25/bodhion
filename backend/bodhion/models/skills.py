...
from bodhion.internal.db import Base, get_db, get_db_context
from bodhion.models.users import Users, UserResponse
...
class Skill(Base):
    ...
    content = Column(Text)
    content_type = Column(String, default="python")  # "python" | "markdown"
...
class SkillForm(BaseModel):
    id: Optional[str] = None   # auto-derived from name if omitted
    name: str
    description: Optional[str] = None
    content: str
    content_type: str = "python"
...
class SkillsTable:
    ...
    def search_skills(
        ...
        try:
            with get_db_context(db) as db:
                from bodhion.models.users import User, UserModel
    ...
    def get_active_markdown_skills_by_user_id(
        self, user_id: str, db: Optional[Session] = None
    ) -> list[SkillUserModel]:
        skills = self.get_skills_by_user_id(user_id, "read", db=db)
        return [
            s for s in skills
            if s.is_active and getattr(s, "content_type", "python") == "markdown"
        ]
