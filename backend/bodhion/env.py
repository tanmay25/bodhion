...
# BODHION_DIR should be the directory where env.py resides (bodhion/)
BODHION_DIR = ENV_FILE_PATH.parent

# BACKEND_DIR is the parent of BODHION_DIR (backend/)
BACKEND_DIR = BODHION_DIR.parent

# BASE_DIR is the parent of BACKEND_DIR (bodhion-dev/)
...
try:
    ...
    # .env.local overrides .env — use for local dev flags (never commit .env.local)
...
if FROM_INIT_PY:
    PACKAGE_DATA = {"version": importlib.metadata.version("bodhion")}
else:
    try:
        # Read version from the React frontend's package.json (BASE_DIR/frontend/package.json).
        # The root package.json is the legacy Svelte app and is no longer maintained.
        PACKAGE_DATA = json.loads((BASE_DIR / "frontend" / "package.json").read_text())
    except Exception:
...
try:
    ...
    changelog_content = (pkgutil.get_data("Bodhion", "CHANGELOG.md") or b"").decode()
...
if FROM_INIT_PY:
    NEW_DATA_DIR = Path(os.getenv("DATA_DIR", BODHION_DIR / "data")).resolve()
    ...
    if DATA_DIR.exists() and DATA_DIR != NEW_DATA_DIR:
        ...
        shutil.make_archive(DATA_DIR.parent / "bodhion_data", "zip", DATA_DIR)

        # Remove the old data directory
        shutil.rmtree(DATA_DIR)

    DATA_DIR = Path(os.getenv("DATA_DIR", BODHION_DIR / "data"))

STATIC_DIR = Path(os.getenv("STATIC_DIR", BODHION_DIR / "static"))

FONTS_DIR = Path(os.getenv("FONTS_DIR", BODHION_DIR / "static" / "fonts"))

FRONTEND_BUILD_DIR = Path(os.getenv("FRONTEND_BUILD_DIR", BASE_DIR / "build")).resolve()

if FROM_INIT_PY:
    FRONTEND_BUILD_DIR = Path(
        os.getenv("FRONTEND_BUILD_DIR", BODHION_DIR / "frontend")
    ).resolve()
...
REDIS_KEY_PREFIX = os.environ.get("REDIS_KEY_PREFIX", "bodhion")
...
ENABLE_CLAUDE_THINKING_API = (
    os.environ.get("ENABLE_CLAUDE_THINKING_API", "false").lower() == "true"
)

_claude_budget_str = os.environ.get("CLAUDE_THINKING_BUDGET_TOKENS", "8000")
try:
    CLAUDE_THINKING_BUDGET_TOKENS = int(_claude_budget_str)
except ValueError:
...
OTEL_SERVICE_NAME = os.environ.get("OTEL_SERVICE_NAME", "bodhion")
