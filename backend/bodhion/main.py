...
# Windows: switch to ProactorEventLoop (IOCP) to avoid the 512-FD select() limit
# that gets hit during embedding generation with many open connections.
...
from bodhion.bodhion_banner import print_banner
from bodhion.utils import logger
from bodhion.utils.audit import AuditLevel, AuditLoggingMiddleware
from bodhion.utils.logger import start_logger
...
from bodhion.internal.db import ScopedSession, engine, get_session

from bodhion.models.functions import Functions
from bodhion.models.models import Models
from bodhion.models.users import UserModel, Users
...
from bodhion.utils.models import (
    get_all_models,
    get_all_base_models,
    check_model_access,
    get_filtered_models,
)
from bodhion.utils.chat import (
    generate_chat_completion as chat_completion_handler,
    chat_completed as chat_completed_handler,
)
from bodhion.utils.actions import chat_action as chat_action_handler
from bodhion.utils.embeddings import generate_embeddings
from bodhion.utils.middleware import (
    build_chat_response_context,
    process_chat_payload,
    process_chat_response,
)
from bodhion.utils.tools import set_tool_servers, set_terminal_servers

from bodhion.utils.auth import (
    get_license_data,
...
from bodhion.utils.plugin import install_tool_and_function_dependencies
from bodhion.utils.seed import seed_default_services
from bodhion.utils.oauth import (
...
from bodhion.utils.security_headers import SecurityHeadersMiddleware
from bodhion.utils.redis import get_redis_connection

from bodhion.tasks import (
...
from bodhion.utils.redis import get_sentinels_from_env
...
async def lifespan(app: FastAPI):
    ...
    # Ensure all default services are present (idempotent — safe on every boot)
    try:
        seed_default_services()
    except Exception as e:
        log.warning(f"[seed] Failed to seed default services at startup: {e}")
...
if ENABLE_OTEL:
    from bodhion.utils.telemetry.setup import setup as setup_opentelemetry
...
app.state.config.SHOW_ADMIN_EVALUATIONS = SHOW_ADMIN_EVALUATIONS
app.state.config.SHOW_ADMIN_INTEGRATIONS = SHOW_ADMIN_INTEGRATIONS
app.state.config.SHOW_ADMIN_WEB_SEARCH = SHOW_ADMIN_WEB_SEARCH
app.state.config.SHOW_ADMIN_CODE_EXECUTION = SHOW_ADMIN_CODE_EXECUTION
app.state.config.SHOW_ADMIN_INTERFACE = SHOW_ADMIN_INTERFACE
app.state.config.SHOW_ADMIN_AUDIO = SHOW_ADMIN_AUDIO
app.state.config.SHOW_ADMIN_IMAGES = SHOW_ADMIN_IMAGES
app.state.config.SHOW_ADMIN_PIPELINES = SHOW_ADMIN_PIPELINES
app.state.config.SHOW_ADMIN_DATABASE = SHOW_ADMIN_DATABASE
...
from bodhion.utils.access_control import migrate_access_control
...
app.state.config.bodhion_native_EXTRACT_IMAGES = bodhion_native_EXTRACT_IMAGES
app.state.config.bodhion_native_OCR_IMAGES = bodhion_native_OCR_IMAGES
app.state.config.bodhion_native_EXTRACT_TABLES = bodhion_native_EXTRACT_TABLES
app.state.config.bodhion_native_IMAGE_MIN_WIDTH = bodhion_native_IMAGE_MIN_WIDTH
app.state.config.bodhion_native_IMAGE_MIN_HEIGHT = bodhion_native_IMAGE_MIN_HEIGHT
app.state.config.BODHION_SECGEM_EXTRACT_IMAGES = BODHION_SECGEM_EXTRACT_IMAGES
app.state.config.BODHION_SECGEM_OCR_IMAGES = BODHION_SECGEM_OCR_IMAGES
app.state.config.BODHION_SECGEM_EXTRACT_TABLES = BODHION_SECGEM_EXTRACT_TABLES
app.state.config.BODHION_SECGEM_IMAGE_MIN_WIDTH = BODHION_SECGEM_IMAGE_MIN_WIDTH
app.state.config.BODHION_SECGEM_IMAGE_MIN_HEIGHT = BODHION_SECGEM_IMAGE_MIN_HEIGHT
app.state.config.BODHION_SECGEM_DETECT_SECS_BLOCKS = BODHION_SECGEM_DETECT_SECS_BLOCKS
app.state.config.BODHION_SECGEM_ANNOTATE_SEMI_TABLES = BODHION_SECGEM_ANNOTATE_SEMI_TABLES
app.state.config.BODHION_SECGEM_PRESERVE_SECTION_CONTEXT = BODHION_SECGEM_PRESERVE_SECTION_CONTEXT
app.state.config.SECGEM_LLM_ENRICHER_OLLAMA_BASE_URL = SECGEM_LLM_ENRICHER_OLLAMA_BASE_URL
app.state.config.SECGEM_LLM_ENRICHER_MODEL = SECGEM_LLM_ENRICHER_MODEL
app.state.config.SECGEM_LLM_ENRICHER_TIMEOUT = SECGEM_LLM_ENRICHER_TIMEOUT
...
app.state.config.RAG_PROCESSOR_PROFILES = RAG_PROCESSOR_PROFILES
app.state.config.RAG_PROCESSOR_KWARGS = RAG_PROCESSOR_KWARGS
app.state.config.RAG_CHUNKING_STRATEGY_MAP = RAG_CHUNKING_STRATEGY_MAP
app.state.config.RAG_CHUNKING_STRATEGY_KWARGS = RAG_CHUNKING_STRATEGY_KWARGS
...
app.state.RERANKING_MODEL_STATUS = {"status": "idle", "model": ""}
        ...
        if request.method == "GET":
            ...
            if "shared" in query_params and len(query_params["shared"]) > 0:
                ...
                if text:
                    ...
                    if urls:
                        from bodhion.retrieval.loaders.youtube import _parse_video_id
...
from bodhion.utils.anthropic import (
            ...
            **(
                {
                    ...
                    "show_admin_evaluations": app.state.config.SHOW_ADMIN_EVALUATIONS,
                    "show_admin_integrations": app.state.config.SHOW_ADMIN_INTEGRATIONS,
                    "show_admin_web_search": app.state.config.SHOW_ADMIN_WEB_SEARCH,
                    "show_admin_code_execution": app.state.config.SHOW_ADMIN_CODE_EXECUTION,
                    "show_admin_interface": app.state.config.SHOW_ADMIN_INTERFACE,
                    "show_admin_audio": app.state.config.SHOW_ADMIN_AUDIO,
                    "show_admin_images": app.state.config.SHOW_ADMIN_IMAGES,
                    "show_admin_pipelines": app.state.config.SHOW_ADMIN_PIPELINES,
                    "show_admin_database": app.state.config.SHOW_ADMIN_DATABASE,
    ...
    try:
        ...
        async with aiohttp.ClientSession(timeout=timeout, trust_env=True) as session:
            async with session.get(
                "https://api.github.com/repos/bodhion/bodhion/releases/latest",
