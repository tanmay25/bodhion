...
from bodhion.utils.payload import (
...
from bodhion.utils.pipeline_steps import (
    ...
    prefixed_with_steps,
    passthrough_with_claude_thinking,
)
from bodhion.utils.anthropic import is_anthropic_url
...
from bodhion.utils.anthropic import is_anthropic_url, get_anthropic_models
...
async def get_models(
    ...
    if url_idx is None:
        ...
        # Fail fast with a clear message when bearer auth is selected but no key is configured.
        # This avoids opaque upstream 500s caused by missing Authorization header.
        if auth_type in ("bearer", None) and not (key or "").strip():
            raise HTTPException(
                status_code=400,
                detail=(
                    f"Missing API key for OpenAI endpoint index {url_idx}. "
                    "Set OPENAI_API_KEYS for this endpoint or change auth_type."
                ),
            )
...
async def generate_chat_completion(
    ...
    # Inject Claude extended thinking parameter when enabled for Anthropic endpoints
    _claude_thinking = ENABLE_CLAUDE_THINKING_API and is_anthropic_url(url)
    if _claude_thinking:
        payload["thinking"] = {
            "type": "enabled",
    ...
    # Build pipeline step events before serialising — needs the dict form of payload.
    # Skip when middleware already emitted steps via socket (progressive_steps flag).
    _progressive = bool((metadata or {}).get("progressive_steps"))
    pipeline_steps = [] if _progressive else build_steps_for_request(payload)
    ...
    try:
        ...
        if "text/event-stream" in r.headers.get("Content-Type", ""):
            streaming = True
            inner = stream_wrapper(r, session, stream_chunks_handler)
            if _claude_thinking:
                inner = passthrough_with_claude_thinking(inner)
            body = inner if _progressive else prefixed_with_steps(pipeline_steps, inner)
            # Strip hop-by-hop / encoding headers: aiohttp auto-decompresses the body
            ...
            _headers = {k: v for k, v in r.headers.items() if k.lower() not in _skip}
