...
from bodhion.models.files import FileModel, FileUpdateForm, Files
from bodhion.utils.access_control.files import has_access_to_file
from bodhion.models.knowledge import Knowledges
from bodhion.storage.provider import Storage
from bodhion.internal.db import get_session, get_db
from sqlalchemy.orm import Session


from bodhion.retrieval.vector.factory import VECTOR_DB_CLIENT

# Document loaders
from bodhion.retrieval.loaders.main import Loader
from bodhion.retrieval.loaders.youtube import YoutubeLoader

# Web search engines
from bodhion.retrieval.web.main import SearchResult
from bodhion.retrieval.web.utils import get_web_loader
from bodhion.retrieval.web.ollama import search_ollama_cloud
from bodhion.retrieval.web.perplexity_search import search_perplexity_search
from bodhion.retrieval.web.brave import search_brave
from bodhion.retrieval.web.kagi import search_kagi
from bodhion.retrieval.web.mojeek import search_mojeek
from bodhion.retrieval.web.bocha import search_bocha
from bodhion.retrieval.web.duckduckgo import search_duckduckgo
from bodhion.retrieval.web.google_pse import search_google_pse
from bodhion.retrieval.web.jina_search import search_jina
from bodhion.retrieval.web.searchapi import search_searchapi
from bodhion.retrieval.web.serpapi import search_serpapi
from bodhion.retrieval.web.searxng import search_searxng
from bodhion.retrieval.web.yacy import search_yacy
from bodhion.retrieval.web.serper import search_serper
from bodhion.retrieval.web.serply import search_serply
from bodhion.retrieval.web.serpstack import search_serpstack
from bodhion.retrieval.web.tavily import search_tavily
from bodhion.retrieval.web.bing import search_bing
from bodhion.retrieval.web.azure import search_azure
from bodhion.retrieval.web.exa import search_exa
from bodhion.retrieval.web.perplexity import search_perplexity
from bodhion.retrieval.web.sougou import search_sougou
from bodhion.retrieval.web.firecrawl import search_firecrawl
from bodhion.retrieval.web.external import search_external
from bodhion.retrieval.web.yandex import search_yandex
from bodhion.retrieval.web.ydc import search_youcom

from bodhion.retrieval.utils import (
...
from bodhion.retrieval.vector.utils import filter_metadata
from bodhion.utils.misc import (
    calculate_sha256_string,
    sanitize_text_for_db,
)
from bodhion.utils.auth import get_admin_user, get_verified_user
from bodhion.utils.access_control import has_permission
...
def get_rf(
    ...
    if reranking_model:
        if any(model in reranking_model for model in ["jinaai/jina-colbert-v2"]):
            try:
                from bodhion.retrieval.models.colbert import ColBERT
            ...
            if engine == "external":
                try:
                    from bodhion.retrieval.models.external import ExternalReranker
...
async def list_processors(user=Depends(get_admin_user)):
    import bodhion.retrieval.processors.builtin  # noqa: F401
    from bodhion.retrieval.processors.registry import available
    return {"processors": available()}


@router.get("/strategies")
async def list_strategies(user=Depends(get_admin_user)):
    import bodhion.retrieval.strategies.builtin  # noqa: F401
    from bodhion.retrieval.strategies.registry import available
...
async def get_rag_config(request: Request, user=Depends(get_admin_user)):
    return {
        ...
        # RAG settings
        "RAG_TEMPLATE": request.app.state.config.RAG_TEMPLATE,
        "RAG_TOP_K": request.app.state.config.TOP_K,
        ...
        "RAG_TOP_K_RERANKER": request.app.state.config.TOP_K_RERANKER,
        "RAG_RELEVANCE_THRESHOLD": request.app.state.config.RELEVANCE_THRESHOLD,
        "RAG_HYBRID_BM25_WEIGHT": request.app.state.config.HYBRID_BM25_WEIGHT,
        ...
        # Bodhion Native Extractor settings
        "bodhion_native_EXTRACT_IMAGES": request.app.state.config.bodhion_native_EXTRACT_IMAGES,
        "bodhion_native_OCR_IMAGES": request.app.state.config.bodhion_native_OCR_IMAGES,
        "bodhion_native_EXTRACT_TABLES": request.app.state.config.bodhion_native_EXTRACT_TABLES,
        "bodhion_native_IMAGE_MIN_WIDTH": request.app.state.config.bodhion_native_IMAGE_MIN_WIDTH,
        "bodhion_native_IMAGE_MIN_HEIGHT": request.app.state.config.bodhion_native_IMAGE_MIN_HEIGHT,
        # Bodhion SecGEM Extractor settings
        "BODHION_SECGEM_EXTRACT_IMAGES": request.app.state.config.BODHION_SECGEM_EXTRACT_IMAGES,
        "BODHION_SECGEM_OCR_IMAGES": request.app.state.config.BODHION_SECGEM_OCR_IMAGES,
        "BODHION_SECGEM_EXTRACT_TABLES": request.app.state.config.BODHION_SECGEM_EXTRACT_TABLES,
        "BODHION_SECGEM_IMAGE_MIN_WIDTH": request.app.state.config.BODHION_SECGEM_IMAGE_MIN_WIDTH,
        "BODHION_SECGEM_IMAGE_MIN_HEIGHT": request.app.state.config.BODHION_SECGEM_IMAGE_MIN_HEIGHT,
        "BODHION_SECGEM_DETECT_SECS_BLOCKS": request.app.state.config.BODHION_SECGEM_DETECT_SECS_BLOCKS,
        "BODHION_SECGEM_ANNOTATE_SEMI_TABLES": request.app.state.config.BODHION_SECGEM_ANNOTATE_SEMI_TABLES,
        "BODHION_SECGEM_PRESERVE_SECTION_CONTEXT": request.app.state.config.BODHION_SECGEM_PRESERVE_SECTION_CONTEXT,
        # SecGEM LLM Enricher settings
        "SECGEM_LLM_ENRICHER_OLLAMA_BASE_URL": request.app.state.config.SECGEM_LLM_ENRICHER_OLLAMA_BASE_URL,
        "SECGEM_LLM_ENRICHER_MODEL": request.app.state.config.SECGEM_LLM_ENRICHER_MODEL,
        "SECGEM_LLM_ENRICHER_TIMEOUT": request.app.state.config.SECGEM_LLM_ENRICHER_TIMEOUT,
        # Reranking settings
        "RAG_RERANKING_MODEL": request.app.state.config.RAG_RERANKING_MODEL,
        ...
        "RAG_PROCESSOR_PROFILES": request.app.state.config.RAG_PROCESSOR_PROFILES,
        "RAG_PROCESSOR_KWARGS": request.app.state.config.RAG_PROCESSOR_KWARGS,
        "RAG_CHUNKING_STRATEGY_MAP": request.app.state.config.RAG_CHUNKING_STRATEGY_MAP,
        "RAG_CHUNKING_STRATEGY_KWARGS": request.app.state.config.RAG_CHUNKING_STRATEGY_KWARGS,
...
class ConfigForm(BaseModel):
    ...
    RAG_TOP_K_RERANKER: Optional[int] = None
    RAG_RELEVANCE_THRESHOLD: Optional[float] = None
    RAG_HYBRID_BM25_WEIGHT: Optional[float] = None
    ...
    # Bodhion Native Extractor settings
    bodhion_native_EXTRACT_IMAGES: Optional[bool] = None
    bodhion_native_OCR_IMAGES: Optional[bool] = None
    bodhion_native_EXTRACT_TABLES: Optional[bool] = None
    bodhion_native_IMAGE_MIN_WIDTH: Optional[int] = None
    bodhion_native_IMAGE_MIN_HEIGHT: Optional[int] = None

    # Bodhion SecGEM Extractor settings (SEMI/GEM/SECS technical documents)
    BODHION_SECGEM_EXTRACT_IMAGES: Optional[bool] = None
    BODHION_SECGEM_OCR_IMAGES: Optional[bool] = None
    BODHION_SECGEM_EXTRACT_TABLES: Optional[bool] = None
    BODHION_SECGEM_IMAGE_MIN_WIDTH: Optional[int] = None
    BODHION_SECGEM_IMAGE_MIN_HEIGHT: Optional[int] = None
    BODHION_SECGEM_DETECT_SECS_BLOCKS: Optional[bool] = None
    BODHION_SECGEM_ANNOTATE_SEMI_TABLES: Optional[bool] = None
    BODHION_SECGEM_PRESERVE_SECTION_CONTEXT: Optional[bool] = None
    # SecGEM LLM Enricher settings
    SECGEM_LLM_ENRICHER_OLLAMA_BASE_URL: Optional[str] = None
    SECGEM_LLM_ENRICHER_MODEL: Optional[str] = None
    SECGEM_LLM_ENRICHER_TIMEOUT: Optional[int] = None
    ...
    # Pipeline configuration
    RAG_PROCESSOR_PROFILES: Optional[dict] = None
    RAG_PROCESSOR_KWARGS: Optional[dict] = None
    RAG_CHUNKING_STRATEGY_MAP: Optional[dict] = None
    RAG_CHUNKING_STRATEGY_KWARGS: Optional[dict] = None
...
async def _load_reranking_model_bg(app) -> None:
    """Download and load the reranker in the background so the save endpoint
    returns immediately. Tracks progress in app.state.RERANKING_MODEL_STATUS."""
    engine  = app.state.config.RAG_RERANKING_ENGINE
    model   = app.state.config.RAG_RERANKING_MODEL
    url     = app.state.config.RAG_EXTERNAL_RERANKER_URL
    key     = app.state.config.RAG_EXTERNAL_RERANKER_API_KEY
    timeout = app.state.config.RAG_EXTERNAL_RERANKER_TIMEOUT

    app.state.RERANKING_MODEL_STATUS = {"status": "loading", "model": model}
    log.info(f"Reranking model load started in background: {model}")
    try:
        rf = await asyncio.to_thread(get_rf, engine, model, url, key, timeout)
        app.state.rf = rf
        app.state.RERANKING_FUNCTION = get_reranking_function(engine, model, rf)
        app.state.RERANKING_MODEL_STATUS = {"status": "ready", "model": model}
        log.info(f"Reranking model ready: {model}")
    except Exception as e:
        log.error(f"Background reranking model load failed: {e}")
        app.state.RERANKING_MODEL_STATUS = {"status": "error", "model": model, "error": str(e)}


@router.get("/config/reranking/status")
async def get_reranking_status(request: Request, user=Depends(get_admin_user)):
    """Return the current reranking model load state so the UI can poll it."""
    return getattr(
        request.app.state,
        "RERANKING_MODEL_STATUS",
        {"status": "idle", "model": ""},
    )
...
async def update_rag_config(
    ...
    request.app.state.config.TOP_K_RERANKER = (
        form_data.RAG_TOP_K_RERANKER
        if form_data.RAG_TOP_K_RERANKER is not None
        else request.app.state.config.TOP_K_RERANKER
    )
    request.app.state.config.RELEVANCE_THRESHOLD = (
        form_data.RAG_RELEVANCE_THRESHOLD
        if form_data.RAG_RELEVANCE_THRESHOLD is not None
        else request.app.state.config.RELEVANCE_THRESHOLD
    )
    request.app.state.config.HYBRID_BM25_WEIGHT = (
        form_data.RAG_HYBRID_BM25_WEIGHT
        if form_data.RAG_HYBRID_BM25_WEIGHT is not None
    ...
    # Bodhion Native Extractor settings
    request.app.state.config.bodhion_native_EXTRACT_IMAGES = (
        form_data.bodhion_native_EXTRACT_IMAGES
        if form_data.bodhion_native_EXTRACT_IMAGES is not None
        else request.app.state.config.bodhion_native_EXTRACT_IMAGES
    )
    request.app.state.config.bodhion_native_OCR_IMAGES = (
        form_data.bodhion_native_OCR_IMAGES
        if form_data.bodhion_native_OCR_IMAGES is not None
        else request.app.state.config.bodhion_native_OCR_IMAGES
    )
    request.app.state.config.bodhion_native_EXTRACT_TABLES = (
        form_data.bodhion_native_EXTRACT_TABLES
        if form_data.bodhion_native_EXTRACT_TABLES is not None
        else request.app.state.config.bodhion_native_EXTRACT_TABLES
    )
    request.app.state.config.bodhion_native_IMAGE_MIN_WIDTH = (
        form_data.bodhion_native_IMAGE_MIN_WIDTH
        if form_data.bodhion_native_IMAGE_MIN_WIDTH is not None
        else request.app.state.config.bodhion_native_IMAGE_MIN_WIDTH
    )
    request.app.state.config.bodhion_native_IMAGE_MIN_HEIGHT = (
        form_data.bodhion_native_IMAGE_MIN_HEIGHT
        if form_data.bodhion_native_IMAGE_MIN_HEIGHT is not None
        else request.app.state.config.bodhion_native_IMAGE_MIN_HEIGHT
    )

    # Bodhion SecGEM Extractor settings
    request.app.state.config.BODHION_SECGEM_EXTRACT_IMAGES = (
        form_data.BODHION_SECGEM_EXTRACT_IMAGES
        if form_data.BODHION_SECGEM_EXTRACT_IMAGES is not None
        else request.app.state.config.BODHION_SECGEM_EXTRACT_IMAGES
    )
    request.app.state.config.BODHION_SECGEM_OCR_IMAGES = (
        form_data.BODHION_SECGEM_OCR_IMAGES
        if form_data.BODHION_SECGEM_OCR_IMAGES is not None
        else request.app.state.config.BODHION_SECGEM_OCR_IMAGES
    )
    request.app.state.config.BODHION_SECGEM_EXTRACT_TABLES = (
        form_data.BODHION_SECGEM_EXTRACT_TABLES
        if form_data.BODHION_SECGEM_EXTRACT_TABLES is not None
        else request.app.state.config.BODHION_SECGEM_EXTRACT_TABLES
    )
    request.app.state.config.BODHION_SECGEM_IMAGE_MIN_WIDTH = (
        form_data.BODHION_SECGEM_IMAGE_MIN_WIDTH
        if form_data.BODHION_SECGEM_IMAGE_MIN_WIDTH is not None
        else request.app.state.config.BODHION_SECGEM_IMAGE_MIN_WIDTH
    )
    request.app.state.config.BODHION_SECGEM_IMAGE_MIN_HEIGHT = (
        form_data.BODHION_SECGEM_IMAGE_MIN_HEIGHT
        if form_data.BODHION_SECGEM_IMAGE_MIN_HEIGHT is not None
        else request.app.state.config.BODHION_SECGEM_IMAGE_MIN_HEIGHT
    )
    request.app.state.config.BODHION_SECGEM_DETECT_SECS_BLOCKS = (
        form_data.BODHION_SECGEM_DETECT_SECS_BLOCKS
        if form_data.BODHION_SECGEM_DETECT_SECS_BLOCKS is not None
        else request.app.state.config.BODHION_SECGEM_DETECT_SECS_BLOCKS
    )
    request.app.state.config.BODHION_SECGEM_ANNOTATE_SEMI_TABLES = (
        form_data.BODHION_SECGEM_ANNOTATE_SEMI_TABLES
        if form_data.BODHION_SECGEM_ANNOTATE_SEMI_TABLES is not None
        else request.app.state.config.BODHION_SECGEM_ANNOTATE_SEMI_TABLES
    )
    request.app.state.config.BODHION_SECGEM_PRESERVE_SECTION_CONTEXT = (
        form_data.BODHION_SECGEM_PRESERVE_SECTION_CONTEXT
        if form_data.BODHION_SECGEM_PRESERVE_SECTION_CONTEXT is not None
        else request.app.state.config.BODHION_SECGEM_PRESERVE_SECTION_CONTEXT
    )

    # SecGEM LLM Enricher settings
    request.app.state.config.SECGEM_LLM_ENRICHER_OLLAMA_BASE_URL = (
        form_data.SECGEM_LLM_ENRICHER_OLLAMA_BASE_URL
        if form_data.SECGEM_LLM_ENRICHER_OLLAMA_BASE_URL is not None
        else request.app.state.config.SECGEM_LLM_ENRICHER_OLLAMA_BASE_URL
    )
    request.app.state.config.SECGEM_LLM_ENRICHER_MODEL = (
        form_data.SECGEM_LLM_ENRICHER_MODEL
        if form_data.SECGEM_LLM_ENRICHER_MODEL is not None
        else request.app.state.config.SECGEM_LLM_ENRICHER_MODEL
    )
    request.app.state.config.SECGEM_LLM_ENRICHER_TIMEOUT = (
        form_data.SECGEM_LLM_ENRICHER_TIMEOUT
        if form_data.SECGEM_LLM_ENRICHER_TIMEOUT is not None
        else request.app.state.config.SECGEM_LLM_ENRICHER_TIMEOUT
    )

    _kwargs = dict(request.app.state.config.RAG_PROCESSOR_KWARGS)
    _kwargs["secgem_llm_enricher"] = {
        "ollama_base_url": request.app.state.config.SECGEM_LLM_ENRICHER_OLLAMA_BASE_URL,
        "model":           request.app.state.config.SECGEM_LLM_ENRICHER_MODEL,
        "timeout":         request.app.state.config.SECGEM_LLM_ENRICHER_TIMEOUT,
    }
    request.app.state.config.RAG_PROCESSOR_KWARGS = _kwargs
    ...
    try:
        ...
        if (
            ...
            asyncio.create_task(
                _load_reranking_model_bg(request.app)
            )
    except Exception as e:
    ...
    # Pipeline configuration
    request.app.state.config.RAG_PROCESSOR_PROFILES = (
        form_data.RAG_PROCESSOR_PROFILES
        if form_data.RAG_PROCESSOR_PROFILES is not None
        else request.app.state.config.RAG_PROCESSOR_PROFILES
    )
    request.app.state.config.RAG_PROCESSOR_KWARGS = (
        form_data.RAG_PROCESSOR_KWARGS
        if form_data.RAG_PROCESSOR_KWARGS is not None
        else request.app.state.config.RAG_PROCESSOR_KWARGS
    )
    request.app.state.config.RAG_CHUNKING_STRATEGY_MAP = (
        form_data.RAG_CHUNKING_STRATEGY_MAP
        if form_data.RAG_CHUNKING_STRATEGY_MAP is not None
        else request.app.state.config.RAG_CHUNKING_STRATEGY_MAP
    )
    request.app.state.config.RAG_CHUNKING_STRATEGY_KWARGS = (
        form_data.RAG_CHUNKING_STRATEGY_KWARGS
        if form_data.RAG_CHUNKING_STRATEGY_KWARGS is not None
        else request.app.state.config.RAG_CHUNKING_STRATEGY_KWARGS
    )
...
def save_docs_to_vector_db(
    ...
    import bodhion.retrieval.processors.builtin  # noqa: F401 — triggers @register decorators
    from bodhion.retrieval.processors.registry import resolve_chain_for_docs

    _processor_kwargs = {
        **request.app.state.config.RAG_PROCESSOR_KWARGS,
        "secgem_llm_enricher": {
            **(request.app.state.config.RAG_PROCESSOR_KWARGS.get("secgem_llm_enricher") or {}),
            "ollama_base_url": request.app.state.config.SECGEM_LLM_ENRICHER_OLLAMA_BASE_URL,
            "model": request.app.state.config.SECGEM_LLM_ENRICHER_MODEL,
            "timeout": request.app.state.config.SECGEM_LLM_ENRICHER_TIMEOUT,
        },
    ...
    chain = resolve_chain_for_docs(
        docs,
        profiles=request.app.state.config.RAG_PROCESSOR_PROFILES,
        processor_kwargs=_processor_kwargs,
    )
    docs = chain.run(docs)

    if split:
        import bodhion.retrieval.strategies.builtin  # noqa: F401 — triggers @register decorators
        from bodhion.retrieval.strategies.registry import resolve_strategy_for_docs

        _cfg = request.app.state.config

        # Live admin-UI kwargs for the two generic strategies.
        # hr_policy_section keeps its own fixed values from RAG_CHUNKING_STRATEGY_KWARGS.
        _strategy_kwargs = {
            **_cfg.RAG_CHUNKING_STRATEGY_KWARGS,
            "character": {
                **_cfg.RAG_CHUNKING_STRATEGY_KWARGS.get("character", {}),
                "chunk_size": _cfg.CHUNK_SIZE,
                "chunk_overlap": _cfg.CHUNK_OVERLAP,
                "min_chunk_size_target": _cfg.CHUNK_MIN_SIZE_TARGET,
                "enable_markdown_headers": _cfg.ENABLE_MARKDOWN_HEADER_TEXT_SPLITTER,
            },
            "token": {
                **_cfg.RAG_CHUNKING_STRATEGY_KWARGS.get("token", {}),
                "chunk_size": _cfg.CHUNK_SIZE,
                "chunk_overlap": _cfg.CHUNK_OVERLAP,
                "tiktoken_encoding": _cfg.TIKTOKEN_ENCODING_NAME,
                "min_chunk_size_target": _cfg.CHUNK_MIN_SIZE_TARGET,
                "enable_markdown_headers": _cfg.ENABLE_MARKDOWN_HEADER_TEXT_SPLITTER,
            },
        }

        # TEXT_SPLITTER admin combo ("character" | "token") drives the default strategy
        # for all loaders that don't have an engine-specific override in the map.
        # bodhion-native-extractor stays locked to hr_policy_section.
        _admin_splitter = _cfg.TEXT_SPLITTER or "character"
        _strategy_map = {
            **_cfg.RAG_CHUNKING_STRATEGY_MAP,
            "default": _admin_splitter,
            "pdf":     _admin_splitter,
            "csv":     _admin_splitter,
            "docx":    _admin_splitter,
        }

        strategy = resolve_strategy_for_docs(
            docs,
            strategy_map=_strategy_map,
            strategy_kwargs=_strategy_kwargs,
        )
        docs = strategy.split(docs)

        if _native_loader is not None and getattr(_native_loader, "_write_output", False):
            _native_loader.write_postprocessed_output(docs)
    ...
    try:
        ...
        if len(embeddings) != len(texts):
            raise ValueError(
                ...
                "The embedding service returned an incomplete or empty response. "
                "Check that the embedding model is reachable and properly configured."
            )
        ...
        try:
            ...
            if form_data.content:
                ...
                if file_path:
                    ...
                    _native_loader_ref = getattr(loader, "_inner_loader", None)
            ...
            if request.app.state.config.BYPASS_EMBEDDING_AND_RETRIEVAL:
                ...
                try:
                    ...
                    result = save_docs_to_vector_db(
                        ...
                        user=user,
                        _native_loader=_native_loader_ref,
                    )
...
def reset_vector_db(user=Depends(get_admin_user), db: Session = Depends(get_session)):
    try:
        ...
        log.exception(f"Failed to reset vector DB: {e}")
        raise HTTPException(status_code=500, detail=str(e))

    success = Knowledges.delete_all_knowledge(db=db)
    if not success:
        log.error("Vector DB was reset but knowledge records could not be deleted from database")
        raise HTTPException(
            status_code=500,
            detail="Vector DB reset but failed to clear knowledge records",
        )
