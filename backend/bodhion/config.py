...
from bodhion.internal.db import Base, get_db
from bodhion.utils.redis import get_redis_connection
...
def run_migrations():
    ...
    try:
        ...
        alembic_cfg = Config(BODHION_DIR / "alembic.ini")

        # Set the script location dynamically
        migrations_path = BODHION_DIR / "migrations"
...
STATIC_DIR = Path(os.getenv("STATIC_DIR", BODHION_DIR / "static")).resolve()
...
def copy_static_asset_if_found(asset_name: str) -> None:
    candidate_paths = [
        FRONTEND_BUILD_DIR / "static" / asset_name,
        BASE_DIR / "static" / asset_name,
        BASE_DIR / "static" / "static" / asset_name,
    ]

    for candidate_path in candidate_paths:
        if candidate_path.exists():
            try:
                shutil.copyfile(candidate_path, STATIC_DIR / asset_name)
                return
            except Exception as e:
                logging.error(f"An error occurred: {e}")


copy_static_asset_if_found("favicon.png")
copy_static_asset_if_found("splash.png")
...
if ENV == "prod":
    if OLLAMA_BASE_URL == "/ollama" and not K8S_FLAG:
        ...
        OLLAMA_BASE_URL = "http://ollama-service.bodhion.svc.cluster.local:11434"
...
SHOW_ADMIN_EVALUATIONS = PersistentConfig(
    "SHOW_ADMIN_EVALUATIONS",
    "auth.admin.tabs.evaluations",
    os.environ.get("SHOW_ADMIN_EVALUATIONS", "false").lower() == "true",
)

SHOW_ADMIN_INTEGRATIONS = PersistentConfig(
    "SHOW_ADMIN_INTEGRATIONS",
    "auth.admin.tabs.integrations",
    os.environ.get("SHOW_ADMIN_INTEGRATIONS", "false").lower() == "true",
)

SHOW_ADMIN_WEB_SEARCH = PersistentConfig(
    "SHOW_ADMIN_WEB_SEARCH",
    "auth.admin.tabs.web_search",
    os.environ.get("SHOW_ADMIN_WEB_SEARCH", "false").lower() == "true",
)

SHOW_ADMIN_CODE_EXECUTION = PersistentConfig(
    "SHOW_ADMIN_CODE_EXECUTION",
    "auth.admin.tabs.code_execution",
    os.environ.get("SHOW_ADMIN_CODE_EXECUTION", "false").lower() == "true",
)

SHOW_ADMIN_INTERFACE = PersistentConfig(
    "SHOW_ADMIN_INTERFACE",
    "auth.admin.tabs.interface",
    os.environ.get("SHOW_ADMIN_INTERFACE", "false").lower() == "true",
)

SHOW_ADMIN_AUDIO = PersistentConfig(
    "SHOW_ADMIN_AUDIO",
    "auth.admin.tabs.audio",
    os.environ.get("SHOW_ADMIN_AUDIO", "false").lower() == "true",
)

SHOW_ADMIN_IMAGES = PersistentConfig(
    "SHOW_ADMIN_IMAGES",
    "auth.admin.tabs.images",
    os.environ.get("SHOW_ADMIN_IMAGES", "false").lower() == "true",
)

SHOW_ADMIN_PIPELINES = PersistentConfig(
    "SHOW_ADMIN_PIPELINES",
    "auth.admin.tabs.pipelines",
    os.environ.get("SHOW_ADMIN_PIPELINES", "false").lower() == "true",
)

SHOW_ADMIN_DATABASE = PersistentConfig(
    "SHOW_ADMIN_DATABASE",
    "auth.admin.tabs.database",
    os.environ.get("SHOW_ADMIN_DATABASE", "false").lower() == "true",
)
...
MILVUS_COLLECTION_PREFIX = os.environ.get("MILVUS_COLLECTION_PREFIX", "bodhion")
...
QDRANT_COLLECTION_PREFIX = os.environ.get("QDRANT_COLLECTION_PREFIX", "bodhion")
...
ELASTICSEARCH_INDEX_PREFIX = os.environ.get(
    "ELASTICSEARCH_INDEX_PREFIX", "bodhion_collections"
)
# Pgvector
...
PINECONE_INDEX_NAME = os.getenv("PINECONE_INDEX_NAME", "bodhion-index")
...
bodhion_native_EXTRACT_IMAGES = PersistentConfig(
    "bodhion_native_EXTRACT_IMAGES",
    "rag.bodhion_native_extract_images",
    os.environ.get("bodhion_native_EXTRACT_IMAGES", "true").lower() == "true",
)

bodhion_native_OCR_IMAGES = PersistentConfig(
    "bodhion_native_OCR_IMAGES",
    "rag.bodhion_native_ocr_images",
    os.environ.get("bodhion_native_OCR_IMAGES", "true").lower() == "true",
)

bodhion_native_EXTRACT_TABLES = PersistentConfig(
    "bodhion_native_EXTRACT_TABLES",
    "rag.bodhion_native_extract_tables",
    os.environ.get("bodhion_native_EXTRACT_TABLES", "true").lower() == "true",
)

bodhion_native_IMAGE_MIN_WIDTH = PersistentConfig(
    "bodhion_native_IMAGE_MIN_WIDTH",
    "rag.bodhion_native_image_min_width",
    int(os.environ.get("bodhion_native_IMAGE_MIN_WIDTH", "50")),
)

bodhion_native_IMAGE_MIN_HEIGHT = PersistentConfig(
    "bodhion_native_IMAGE_MIN_HEIGHT",
    "rag.bodhion_native_image_min_height",
    int(os.environ.get("bodhion_native_IMAGE_MIN_HEIGHT", "50")),
)
...
BODHION_SECGEM_EXTRACT_IMAGES = PersistentConfig(
    "BODHION_SECGEM_EXTRACT_IMAGES",
    "rag.BODHION_SECGEM_extract_images",
    os.environ.get("BODHION_SECGEM_EXTRACT_IMAGES", "true").lower() == "true",
)

BODHION_SECGEM_OCR_IMAGES = PersistentConfig(
    "BODHION_SECGEM_OCR_IMAGES",
    "rag.BODHION_SECGEM_ocr_images",
    os.environ.get("BODHION_SECGEM_OCR_IMAGES", "true").lower() == "true",
)

BODHION_SECGEM_EXTRACT_TABLES = PersistentConfig(
    "BODHION_SECGEM_EXTRACT_TABLES",
    "rag.BODHION_SECGEM_extract_tables",
    os.environ.get("BODHION_SECGEM_EXTRACT_TABLES", "true").lower() == "true",
)

BODHION_SECGEM_IMAGE_MIN_WIDTH = PersistentConfig(
    "BODHION_SECGEM_IMAGE_MIN_WIDTH",
    "rag.BODHION_SECGEM_image_min_width",
    int(os.environ.get("BODHION_SECGEM_IMAGE_MIN_WIDTH", "50")),
)

BODHION_SECGEM_IMAGE_MIN_HEIGHT = PersistentConfig(
    "BODHION_SECGEM_IMAGE_MIN_HEIGHT",
    "rag.BODHION_SECGEM_image_min_height",
    int(os.environ.get("BODHION_SECGEM_IMAGE_MIN_HEIGHT", "50")),
)

BODHION_SECGEM_DETECT_SECS_BLOCKS = PersistentConfig(
    "BODHION_SECGEM_DETECT_SECS_BLOCKS",
    "rag.BODHION_SECGEM_detect_secs_blocks",
    os.environ.get("BODHION_SECGEM_DETECT_SECS_BLOCKS", "true").lower() == "true",
)

BODHION_SECGEM_ANNOTATE_SEMI_TABLES = PersistentConfig(
    "BODHION_SECGEM_ANNOTATE_SEMI_TABLES",
    "rag.BODHION_SECGEM_annotate_semi_tables",
    os.environ.get("BODHION_SECGEM_ANNOTATE_SEMI_TABLES", "true").lower() == "true",
)

BODHION_SECGEM_PRESERVE_SECTION_CONTEXT = PersistentConfig(
    "BODHION_SECGEM_PRESERVE_SECTION_CONTEXT",
    "rag.BODHION_SECGEM_preserve_section_context",
    os.environ.get("BODHION_SECGEM_PRESERVE_SECTION_CONTEXT", "true").lower() == "true",
)

SECGEM_LLM_ENRICHER_OLLAMA_BASE_URL = PersistentConfig(
    "SECGEM_LLM_ENRICHER_OLLAMA_BASE_URL",
    "rag.secgem_llm_enricher_ollama_base_url",
    os.environ.get("SECGEM_LLM_ENRICHER_OLLAMA_BASE_URL", "http://localhost:11434"),
)

SECGEM_LLM_ENRICHER_MODEL = PersistentConfig(
    "SECGEM_LLM_ENRICHER_MODEL",
    "rag.secgem_llm_enricher_model",
    os.environ.get("SECGEM_LLM_ENRICHER_MODEL", "mistral"),
)

SECGEM_LLM_ENRICHER_TIMEOUT = PersistentConfig(
    "SECGEM_LLM_ENRICHER_TIMEOUT",
    "rag.secgem_llm_enricher_timeout",
    int(os.environ.get("SECGEM_LLM_ENRICHER_TIMEOUT", "30")),
)
...
You are Bodhion, Bodhion HR policy assistant. You help employees understand company policies based solely on the official documents provided.

### Guidelines:
- Answer **only** from the content inside the `<context>` block. Do not use outside knowledge for policy specifics — amounts, eligibility criteria, dates, and grade levels in your training data may be outdated or incorrect.
- If the context does not contain enough information, clearly say so and advise the employee to contact HR directly.
- If the query is outside HR policy scope (e.g. general or technical questions), politely state you can only assist with HR policy queries.
- If multiple policies are relevant, address each clearly and state which policy the information comes from.
- Ask for clarification only when genuinely needed (e.g. employee grade or travel type changes the answer).
- Respond in the same language as the user's query.
- Do not use XML tags in your response.

### Response Format:
- Use bullet points or numbered steps for eligibility criteria, processes, or multi-part answers.
- Use **bold** for key values: amounts, dates, deadlines, and grade levels.
- Keep responses concise — do not repeat the question back.
- Cite every factual claim using [id] only when the `<source>` tag includes an explicit `id` attribute.

### Example of Citation:
* "Employees at grade **S4 and above** are eligible to nominate reportees for the Quarter Champ award [1]."

### Output:
A clear, structured answer grounded strictly in the provided context, with inline citations [id] where applicable.
...
RAG_PROCESSOR_PROFILES = PersistentConfig(
    ...
    "rag.processor_profiles",
    {
        ...
        "pdf": ["strip_image_placeholders", "filter_min_content"],
        "csv": [],
        "docx": ["filter_min_content"],
    },
)
...
RAG_CHUNKING_STRATEGY_MAP = PersistentConfig(
    ...
    "rag.chunking_strategy_map",
    {
        ...
        "bodhion-secgem-extractor": "secgem",
        "pdf": "character",
        "csv": "character",
        "docx": "character",
    },
