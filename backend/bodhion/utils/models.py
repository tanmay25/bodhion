...
from bodhion.socket.utils import RedisDict
from bodhion.routers import openai, ollama
from bodhion.functions import get_function_models
...
from bodhion.utils.access_control import has_access
...
from bodhion.env import BYPASS_MODEL_ACCESS_CONTROL, GLOBAL_LOG_LEVEL
...
def is_non_llm_model(model: dict) -> bool:
    model_id = (model.get("id") or model.get("model") or model.get("name") or "").lower()

    capabilities = model.get("info", {}).get("meta", {}).get("capabilities") or {}
    if (
        capabilities.get("completion") is False
        and capabilities.get("chat_completion") is False
    ):
        return True

    raw_model = model.get("ollama") or model
    details = raw_model.get("details") or {}

    detail_values = []
    for key in ("family", "parent_model"):
        value = details.get(key)
        if value:
            detail_values.append(str(value).lower())

    for value in details.get("families") or []:
        if value:
            detail_values.append(str(value).lower())
    ...
    searchable_text = " ".join([model_id, *detail_values])
    return any(marker in searchable_text for marker in non_llm_markers)
...
async def fetch_ollama_models(request: Request, user: UserModel = None):
    ...
    return [model for model in models if not is_non_llm_model(model)]
