...
from bodhion.utils.misc import get_last_user_message, get_messages_content

from bodhion.config import DEFAULT_RAG_TEMPLATE
...
def rag_template(template: str, context: str, query: str):
    ...
    has_context_ph = "[context]" in template or "{{CONTEXT}}" in template
    log.debug(
        f"[RAG CP6] rag_template | template_len={len(template)} context_len={len(context)} "
        f"query_len={len(query)} has_context_placeholder={has_context_ph}"
    )

    if "[context]" not in template and "{{CONTEXT}}" not in template:
        log.debug(
            "[RAG CP6] WARNING — RAG_TEMPLATE has no [context] or {{CONTEXT}} placeholder! "
            "Retrieved content will NOT be injected into the prompt."
        )
