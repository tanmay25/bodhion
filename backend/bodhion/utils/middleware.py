...
from bodhion.utils.misc import is_string_allowed
from bodhion.models.oauth_sessions import OAuthSessions
from bodhion.models.chats import Chats
from bodhion.models.folders import Folders
from bodhion.models.users import Users
from bodhion.socket.main import (
    get_event_call,
    get_event_emitter,
)
from bodhion.routers.tasks import (
    generate_queries,
    generate_title,
    generate_follow_ups,
...
from bodhion.routers.retrieval import (
    process_web_search,
    SearchForm,
)
from bodhion.utils.tools import get_builtin_tools
from bodhion.routers.images import (
    image_generations,
    CreateImageForm,
    image_edits,
    EditImageForm,
)
from bodhion.routers.pipelines import (
    process_pipeline_inlet_filter,
    process_pipeline_outlet_filter,
)
from bodhion.routers.memories import query_memory, QueryMemoryForm

from bodhion.utils.webhook import post_webhook
from bodhion.utils.files import (
...
from bodhion.models.users import UserModel
from bodhion.models.functions import Functions
from bodhion.models.models import Models

from bodhion.retrieval.utils import get_sources_from_items


from bodhion.utils.sanitize import sanitize_code
from bodhion.utils.pipeline_steps import (
    make_step,
    STEP_PREPARING,
    STEP_RETRIEVING,
    STEP_RERANKING,
...
from bodhion.utils.chat import generate_chat_completion
from bodhion.utils.task import (
    get_task_model_id,
    rag_template,
...
from bodhion.utils.access_control import has_connection_access
from bodhion.utils.plugin import load_function_module_by_id
from bodhion.utils.filter import (
    get_sorted_filter_ids,
    process_filter_functions,
)
from bodhion.utils.code_interpreter import execute_code_jupyter
from bodhion.utils.payload import apply_system_prompt_to_body
from bodhion.utils.response import normalize_usage
...
from bodhion.utils.headers import include_user_info_headers
from bodhion.constants import TASKS
...
def serialize_output(output: list) -> str:
    ...
    for idx, item in enumerate(output):
        ...
        elif item_type == "bodhion:code_interpreter":
...
def apply_source_context_to_messages(
    ...
    """
    log.debug(
        f"[RAG CP5] apply_source_context_to_messages called | "
        f"sources={len(sources)} user_message_present={bool(user_message)} "
        f"include_content={include_content} RAG_SYSTEM_CONTEXT={RAG_SYSTEM_CONTEXT}"
    )

    if not sources or not user_message:
        log.debug(
            f"[RAG CP5] EARLY EXIT — sources empty={not sources} user_message empty={not user_message}"
        )
        return messages
    ...
    for source in sources:
        for doc, meta in zip(source.get("document", []), source.get("metadata", [])):
            ...
            log.debug(
                f"[RAG CP5] source chunk | src_id={src_id} src_name={src_name} "
                f"doc_len={len(doc) if doc else 0} body_len={len(body)}"
            )

    context_string = context_string.strip()
    log.debug(
        f"[RAG CP5] context_string built | len={len(context_string)} empty={not context_string}"
    )

    if not context_string:
        log.debug("[RAG CP5] EARLY EXIT — context_string is empty after build")
        return messages

    rag_tmpl = request.app.state.config.RAG_TEMPLATE
    has_context_placeholder = "[context]" in rag_tmpl or "{{CONTEXT}}" in rag_tmpl
    log.debug(
        f"[RAG CP5] RAG_TEMPLATE len={len(rag_tmpl)} "
        f"has_context_placeholder={has_context_placeholder} "
        f"injecting_into={'system' if RAG_SYSTEM_CONTEXT else 'user_message'}"
    )
...
async def chat_completion_files_handler(
    ...
    log.debug(
        f"[RAG CP2] chat_completion_files_handler entry | "
        f"metadata_files={body.get('metadata', {}).get('files')}"
    )

    if files := body.get("metadata", {}).get("files", None):
        log.debug(
            f"[RAG CP2] files present | count={len(files)} "
            f"items={[{'type': f.get('type'), 'id': f.get('id'), 'name': f.get('name'), 'context': f.get('context'), 'collection_name': f.get('collection_name')} for f in files]}"
        )

        # Check if all files are in full context mode
        all_full_context = all(item.get("context") == "full" for item in files)
        log.debug(f"[RAG CP2] all_full_context={all_full_context}")
        ...
        log.debug(
            f"[RAG CP2] queries for retrieval | count={len(queries)} queries={queries}"
        )
        log.debug(
            f"[RAG CP2] retrieval config | "
            f"TOP_K={request.app.state.config.TOP_K} "
            f"RELEVANCE_THRESHOLD={request.app.state.config.RELEVANCE_THRESHOLD} "
            f"ENABLE_RAG_HYBRID_SEARCH={request.app.state.config.ENABLE_RAG_HYBRID_SEARCH} "
            f"RAG_FULL_CONTEXT={request.app.state.config.RAG_FULL_CONTEXT} "
            f"full_context_this_call={all_full_context or request.app.state.config.RAG_FULL_CONTEXT} "
            f"EMBEDDING_FUNCTION_present={bool(request.app.state.EMBEDDING_FUNCTION)} "
            f"RERANKING_FUNCTION_present={bool(request.app.state.RERANKING_FUNCTION)}"
        )
        ...
        try:
            ...
            log.debug(
                f"[RAG CP2] get_sources_from_items returned | sources_count={len(sources)} "
                f"source_names={[s.get('source', {}).get('name') for s in sources]} "
                f"doc_counts={[len(s.get('document', [])) for s in sources]}"
            )
        except Exception as e:
            log.debug(f"[RAG CP2] get_sources_from_items EXCEPTION: {e}")
            log.exception(e)
...
async def _emit_step(event_emitter, step: dict) -> None:
    """Emit a pipeline step via socket.io so the UI can show progress before LLM streaming starts."""
    if event_emitter:
        try:
            await event_emitter({"type": "chat:step", "data": step})
        except Exception:
            pass
...
async def process_chat_payload(request, form_data, user, metadata, model):
    ...
    # Emit STEP_PREPARING started as early as possible so the UI shows activity
    _prep_step_id = str(uuid4())
    await _emit_step(event_emitter, make_step(STEP_PREPARING, "Preparing request…", "started", step_id=_prep_step_id))
    ...
    log.debug(
        f"[RAG CP1] process_chat_payload entry | model_id={model.get('id')} "
        f"model_knowledge={bool(model_knowledge)} knowledge_items={model_knowledge} "
        f"chat_id={metadata.get('chat_id')} "
        f"files_in_form_data={[f.get('name', f.get('id')) for f in form_data.get('files', [])]} "
    ...
    # Inlet filters done → preparing step complete
    await _emit_step(event_emitter, make_step(STEP_PREPARING, "Request prepared", "done", step_id=_prep_step_id))

    _features_raw = form_data.get("features") or {}
    if _features_raw.get("web_search"):
        form_data.setdefault("metadata", {})["pipeline_web_search"] = True
    ...
    if features:
        ...
        if "web_search" in features and features["web_search"]:
            ...
            if metadata.get("params", {}).get("function_calling") != "native":
                _web_step_id = str(uuid4())
                await _emit_step(event_emitter, make_step(STEP_RETRIEVING, "Searching the web…", "started", step_id=_web_step_id))
                form_data = await chat_web_search_handler(
                    request, form_data, extra_params, user
                )
                await _emit_step(event_emitter, make_step(STEP_RETRIEVING, "Web results retrieved", "done", step_id=_web_step_id))
    ...
    _rag_files_raw = [
        {"collection_name": f.get("collection_name"), "name": f.get("name", "")}
        for f in (form_data.get("files") or [])
        if isinstance(f, dict) and f.get("collection_name")
    ]
    if _rag_files_raw:
        form_data.setdefault("metadata", {})["pipeline_rag_files"] = _rag_files_raw
    ...
    available_skills = []
    if all_skill_ids:
        from bodhion.models.skills import Skills as SkillsModel
        ...
        for skill in available_skills:
            if skill.content_type == "markdown" or skill.id in user_skill_ids:
                # Markdown skills always inject full content (they are context/knowledge).
                # User-selected python skills also get full content.
                form_data["messages"] = add_or_update_system_message(
                    f'<skill name="{skill.name}" type="{skill.content_type}">\n{skill.content}\n</skill>',
                    form_data["messages"],
                    append=True,
                )
            else:
                # Model-attached python skill: expose name+description as a callable stub.
    ...
    log.debug(
        f"[RAG CP3] file_context gate | file_context_enabled={file_context_enabled} "
        f"files_in_metadata={[f.get('name', f.get('id')) for f in (metadata.get('files') or [])]}"
    )

    if file_context_enabled:
        # Detect RAG files to show retrieval progress steps
        _rag_files_meta = [f for f in (metadata.get("files") or []) if f and f.get("collection_name")]
        _rag_count = len(_rag_files_meta)
        _rag_step_id = None
        if _rag_count:
            _rag_label = f"{_rag_count} document{'s' if _rag_count > 1 else ''}"
            _rag_step_id = str(uuid4())
            await _emit_step(event_emitter, make_step(STEP_RETRIEVING, f"Searching {_rag_label}…", "started", step_id=_rag_step_id))

        try:
            form_data, flags = await chat_completion_files_handler(
                request, form_data, extra_params, user
            )
            handler_sources = flags.get("sources", [])
            log.debug(
                f"[RAG CP3] files_handler returned | sources_count={len(handler_sources)} "
                f"source_names={[s.get('source', {}).get('name') for s in handler_sources]}"
            )
            sources.extend(handler_sources)
        except Exception as e:
            log.debug(f"[RAG CP3] files_handler EXCEPTION: {e}")
            log.exception(e)

        if _rag_step_id:
            await _emit_step(event_emitter, make_step(STEP_RETRIEVING, f"Retrieved context from {_rag_label}", "done", step_id=_rag_step_id))
            _rerank_id = str(uuid4())
            await _emit_step(event_emitter, make_step(STEP_RERANKING, "Re-ranking results…", "started", step_id=_rerank_id))
            await _emit_step(event_emitter, make_step(STEP_RERANKING, "Results ranked", "done", step_id=_rerank_id))
            _sel_id = str(uuid4())
            await _emit_step(event_emitter, make_step(STEP_SELECTING, "Selecting best context…", "started", step_id=_sel_id))
            await _emit_step(event_emitter, make_step(STEP_SELECTING, "Context selected", "done", step_id=_sel_id))
    else:
        log.debug("[RAG CP3] SKIPPED — file_context_enabled=False on this model")

    log.debug(
        f"[RAG CP4] pre-inject check | total_sources={len(sources)} prompt_present={bool(prompt)}"
    )
    ...
    if sources and prompt:
        ...
        log.debug(
            f"[RAG CP4] SKIPPED inject — sources empty={not sources} prompt empty={not prompt}"
        )
    ...
    # Signal that step events were emitted via socket; LLM routers should skip SSE prefix injection
    await _emit_step(event_emitter, make_step(STEP_GENERATING, "Generating response…", "started"))
    metadata["progressive_steps"] = True
    form_data["metadata"] = metadata
        ...
        async def response_handler(response, events):
            def tag_output_handler(content_type, tags, output):
                ...
                output_type_map = {
                    ...
                    "code_interpreter": "bodhion:code_interpreter",
                }
                ...
                if last_type == "message":
                    ...
                    for start_tag, end_tag in tags:
                        ...
                        if match:
                            ...
                            elif output_item_type == "bodhion:code_interpreter":
                                output.append(
                                    {
                                        "type": "bodhion:code_interpreter",
                                        "id": output_id("ci"),
                                        "status": "in_progress",
                                        "start_tag": start_tag,
                                        "end_tag": end_tag,
                    ...
                    or (
                        last_type == "bodhion:code_interpreter"
                    ...
                    elif last_type == "bodhion:code_interpreter":
            ...
            try:
                ...
                if DETECT_CODE_INTERPRETER:
                    ...
                    while (
                        output
                        and output[-1].get("type") == "bodhion:code_interpreter"
